...
def exp_config():
    ...
    output_dir = "all_saves/htcn_mixed_wd_bin"
...
def exp_htcn_mixed(cfg_file, output_dir, dataset_source, m_dataset_target,
    ...
    session = 1
    fasterRCNN, lr, optimizer, session, start_epoch, optimizer_wd = init_htcn_model_optimizer(LA_ATT, MID_ATT, class_agnostic, device, gc,
                                                                                              imdb, lc, load_name, net, optimizer, resume,
                                                                                              session, start_epoch, with_wd=True)
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        total_step = frcnn_utils.train_htcn_one_epoch_multi_targets_seq_wd_2_target(args, FL, total_step, dataloader_s, m_dataloader_t, iters_per_epoch, fasterRCNN, optimizer, device, critic, critic_optim, optimizer_wd, logger)
...
if __name__ == "__main__":
    ...
    ex.run(config_updates={'cfg_file': 'cfgs/vgg16.yml'},
           options={"--name": 'htcn_wd_bin_kitti_2_city_water_vgg16'})