...
def exp_config():
    ...
    output_dir = "all_saves/htcn_mixed_seq_bin"
    dataset_source = "voc_0712"
    m_dataset_target = ["comic", "clipart", "watercolor"]
...
def exp_htcn_mixed(cfg_file, output_dir, dataset_source, m_dataset_target,
    ...
    fasterRCNN, lr, optimizer, session, start_epoch, _ = init_htcn_model_optimizer(lr, LA_ATT, MID_ATT, class_agnostic, device, gc,
                                                                                   ...
                                                                                   session, start_epoch, len(m_dataset_target))
    ...
    if ef:
        ...
        FL = FocalLoss(class_num=len(m_dataset_target) + 1, gamma=gamma)
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        total_step = frcnn_utils.train_htcn_one_epoch_multi_targets_seq_mce_binary(args, FL, total_step, dataloader_s, m_dataloader_t, iters_per_epoch, fasterRCNN, optimizer, device, True, logger)
...
if __name__ == "__main__":
    ...
    ex.run(config_updates={'cfg_file': 'cfgs/vgg16.yml'},
           options={"--name": 'htcn_seq_bin_mce_mtda_kitti_2_city_water_vgg16'})