...
def exp_config():
    ...
    output_dir = "all_saves/htcn_mixed_seq_bin_mrpn"
...
@ex.capture
def exp_htcn_mixed(cfg_file, output_dir, dataset_source, m_dataset_target, is_mtda,
    ...
    args_val = Args(dataset=dataset_source, dataset_t=m_dataset_target, imdb_name_target=[], cfg_file=cfg_file, net=net)
    ...
    m_val_dataloader_t, m_val_imdb_t = init_val_dataloaders_mt(args_val, batch_size, num_workers)

    session = 1
    num_target = 1
    if is_mtda:
        num_target = len(m_dataset_target)

    fasterRCNN, lr, optimizer, session, start_epoch, _ = init_mrpn_model_optimizer(lr, LA_ATT, MID_ATT, class_agnostic, device, gc,
                                                                           imdb, lc, load_name, net, optimizer, resume,
                                                                           session, start_epoch, 1, is_mtda)
    ...
    if ef:
        FL = EFocalLoss(class_num=num_target + 1, gamma=gamma)
    else:
        FL = FocalLoss(class_num=num_target + 1, gamma=gamma)
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        total_step = frcnn_utils.train_htcn_one_epoch_multi_targets_seq_mce_binary_pw(args, FL, total_step, dataloader_s, m_dataloader_t, iters_per_epoch, fasterRCNN, optimizer, device, False, is_mtda, logger)

        avg_ap = 0
        for i, val_dataloader_t in enumerate(m_val_dataloader_t):
            map = frcnn_utils.eval_one_dataloader(output_dir, val_dataloader_t, fasterRCNN, device, m_val_imdb_t[i], target_num=0)
...
if __name__ == "__main__":
    ...
    ex.run(config_updates={'cfg_file': 'cfgs/vgg16.yml',
                           'is_mtda': False},
           options={"--name": 'htcn_seq_bin_kitti_2_city_water_vgg16_real_1rpn_pairwise'})