...
class comic(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'comic' + year + '_' + image_set)
        self._year = year
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            # x1 = max(float(bbox.find('xmin').text) - 1, 0)
            # y1 = max(float(bbox.find('ymin').text) - 1, 0) #Difference because some bbox is of ymin=1, ymax=1
            # x2 = min(float(bbox.find('xmax').text), w)
            # y2 = min(float(bbox.find('ymax').text), h)
            #
