...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        labels = gt_boxes.new(batch_size, inds_inside.size(0)).fill_(-1)  #[1,n]
        bbox_inside_weights = gt_boxes.new(batch_size, inds_inside.size(0)).zero_() #[1,n]
        bbox_outside_weights = gt_boxes.new(batch_size, inds_inside.size(0)).zero_() #[1,n]

        overlaps = bbox_overlaps_batch(anchors, gt_boxes) #[1, n, 20]
        ...
        max_overlaps, argmax_overlaps = torch.max(overlaps, 2) # [1, n] [1,n]
        gt_max_overlaps, _ = torch.max(overlaps, 1)   # [1,20]

        if not cfg.TRAIN.RPN_CLOBBER_POSITIVES:
            labels[max_overlaps < cfg.TRAIN.RPN_NEGATIVE_OVERLAP] = 0

        gt_max_overlaps[gt_max_overlaps==0] = 1e-5
        keep = torch.sum(overlaps.eq(gt_max_overlaps.view(batch_size,1,-1).expand_as(overlaps)), 2) #[1, n]
...
#           num_bg = cfg.TRAIN.RPN_BATCHSIZE - sum_fg[i]
        ...
        # add for mask for source distillation
