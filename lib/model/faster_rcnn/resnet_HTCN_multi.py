...
from model.faster_rcnn.faster_rcnn_HTCN_multi import _fasterRCNN
...
class netD_pixel(nn.Module):
    def __init__(self,context=False, in_chan=256, target_num=1):
        ...
        self.conv2 = nn.Conv2d(256, 128, kernel_size=1, stride=1,
                               padding=0, bias=False)
        self.conv3 = nn.Conv2d(128, target_num, kernel_size=1, stride=1,
                               padding=0, bias=False)
...
class netD_mid(nn.Module):
    def __init__(self,context=False, in_chan=512, target_num=1):
        ...
        self.fc = nn.Linear(128,target_num + 1)
        self.context = context
    def forward(self, x):
...
class netD(nn.Module):
    def __init__(self,context=False, in_chan=1024, target_num=1):
...
class netD_dc(nn.Module):
    def __init__(self, target_num=1):
        ...
        self.fc3 = nn.Linear(100,target_num+1)
    def forward(self, x):
...
class netD_da(nn.Module):
    def __init__(self, feat_d, target_num=1):
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False,lc=False,gc=False, la_attention = False
               ,mid_attention = False, target_num=1):
  ...
  def _init_modules(self):
    ...
    if self.layers in [50, 101, 152]:
        self.netD_pixel = netD_pixel(context=self.lc, target_num=self.target_num)
        self.netD = netD(context=self.gc, target_num=self.target_num)
        self.netD_mid = netD_mid(context=self.gc, target_num=self.target_num)
        feat_d = 2048
    else:
        self.netD_pixel = netD_pixel(context=self.lc, in_chan=in_chan, target_num=self.target_num)
        self.netD = netD(context=self.gc, in_chan=256, target_num=self.target_num)
        self.netD_mid = netD_mid(context=self.gc, in_chan=in_chan*2, target_num=self.target_num)
        feat_d = 512
    ...
    self.netD_da = netD_da(feat_d3,target_num=self.target_num)
  ...
  def re_init_da_layers(self, device):
      ...
      self.netD_da = netD_da(feat_d3, target_num=self.target_num + 1).to(device)
